...
patients %>% 
  ...
  mutate(pat_id= 1:nrow(.)) %>% #(.)whatever you just piped to to infinity pretty much.
...
saveRDS(df, "C:/Users/User/Data_Course_INGRAM/Data/clean_bp.RDS")
...
#10/12/22
#code that Geoff sent us:

library(tidyverse)
sentences # built in vector of sentences
remove_vowels <- function(x){x %>% str_remove_all("a|e|i|o|u")} # make a new function
remove_vowels(sentences) # use the function
